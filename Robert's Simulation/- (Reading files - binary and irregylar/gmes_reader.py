...
def data_reader(bufferStream, nsat :int , nmes :int, off_set =0):
    ...
    dtype = np.dtype('4i1, 2i4, {}f8,i2'.format(nmes))                 # This is our format for a gmes data record
...
def save_to_text (fname, npArr):
    ...
    ntype = np.dtype(fmt)
    np_cleaner = np.asarray(npArr,dtype = ntype)
    np.savetxt(fname, npArr, fmt='%.18e', delimiter=' ', newline='\n')
